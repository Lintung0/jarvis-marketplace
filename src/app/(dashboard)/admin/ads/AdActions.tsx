...
import { toggleAd, deleteAd } from "@/app/actions/ads"
import { AdForm } from "@/components/admin/AdForm"
import type { AdSpace } from "@/types"

export function AdActions({ ad }: { ad: AdSpace }) {
  const [active, setActive] = useState(ad.is_active)
  const [editing, setEditing] = useState(false)

  async function handleToggle() {
    const newVal = !active
    setActive(newVal)
    try { await toggleAd(ad.id) }
    catch { setActive(active) }
  }

  async function handleDelete() {
    if (!confirm("Delete this ad space?")) return
    try { await deleteAd(ad.id) }
